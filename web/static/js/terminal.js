	...
	if (_is_mobile) {
		...
		terminalOpen = false;
		function launchInnoRobotsTerminal() {
			if (terminalOpen) {
				terminalOpen = false;
				$("#onlineTerminal").html('');
...
"\n"+"#################"+
					...
					"\n Welcome to the Innorobots Terminal."+breakLine()+"Try <strong>help()</strong> "+breakLine()+breakLine()+breakLine();
		...
		$("#onlineTerminal").on('keydown', function(e) {
			...
			if (e.keyCode == 9) {
				e.preventDefault();
				var toAutocomplete = '';
				var ar_functionlist = Array('clear','innorobots' , 'cls', 'doge', 'dancer', 'help', 'planetarium', 'scrollTo', 'comicSans', 'helvetica', 'ieLove', 'weather', 'exit', 'lipsum', 'fbShare', 'twShare', 'latestTweets', 'likeasir', 'shades', 'perspective');
				var ar_terminal_lines = $("#onlineTerminal").html().split("\n");
		...
		function innorobots() { $("#onlineTerminal").html($("#onlineTerminal").html().trim()+'<span class="response"> dont miss innorobots event ;)</span>'+breakLine()+breakLine());}
		...
		function doge(func_options) {
			...
			if (func_options == "clear") {
				...
				av_doge_texts = Array('wow', 'much code', 'much InnoRobots', 'so InnoRobots', 'so code', 'very InnoRobots', 'very programmer', 'd0g3 l337 h4x0r', 'wow', 'wow');
		...
		function help(customFunctionName) {
			if (typeof customFunctionName === "undefined") {
				...
				if (customFunctionName == 'clear')        response = 'Get a fresh start, removing all text from InnoRobots Terminal.';
				if (customFunctionName == 'cls')          response = 'Get a fresh start, removing all text from InnoRobots Terminal.';
				...
				if (customFunctionName == 'exit')         response = 'It closes the close InnoRobots Terminal.';
				if (customFunctionName == 'fbShare')      response = 'Share InnoRobotsConf with you friends on Facebook.';
				...
				if (customFunctionName == 'latestTweets') response = 'Get the latest tweets from @InnoRobotsconfar.';
				...
				if (customFunctionName == 'twShare')      response = 'Share InnoRobotsConf with you friends on Twitter.';
				if (customFunctionName == 'weather')      response = 'Shows the weather forecast for the two days of InnoRobotsConf 2014. It will be available only 5 days before the event.';
			}
		...
		function weather() {
			var fiveDaysBeforeInnoRobotsConf = new Date(2014, 11, 24);
			var today = new Date();
			//if (today.getTime() < fiveDaysBeforeInnoRobotsConf.getTime()) {
			...
			//} else {
				$("#onlineTerminal").html($("#onlineTerminal").html().trim()+'<span class="response">Loading weather condition for InnoRobotsConf 2014 Buenos Aires...</span>'+breakLine()+breakLine());
				var uri = "//api.worldweatheronline.com/free/v1/weather.ashx?q=Buenos%20Aires&format=InnoRobotson&num_of_days=2&date=2014-11-29&lang=es&key=c0bc674c45172b0ab46399b10bd4c0e1bf532d17";
				$.getInnoRobotsON(uri, function(data) {
		...
		function fbShare() {
			...
			window.open('https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fwww.InnoRobotsconfar.com%2F', 'sharer', 'top='+winTop+', left='+winLeft+', toolbar=0, status=0, width='+winWidth+', height='+winHeight, true);
		...
		function twShare() {
			...
			var winTop = ((screen.height-winHeight) / 2) - (winHeight / 2);
			var winLeft = ((screen.width-winWidth) / 2) - (winWidth / 2);
			window.open('https://twitter.com/home?status=InnoRobotsConf+Buenos+Aires+2014+-+Conferences+for+the+Javascript+Community+http%3A%2F%2Fwww.InnoRobotsconfar.com%2F', 'sharer', 'top='+winTop+',left='+winLeft+',toolbar=0,status=0,width='+winWidth+',height='+winHeight, true);
			$("#onlineTerminal").html($("#onlineTerminal").html().trim()+'<span class="response">Please allow popups in your browser</span>'+breakLine()+breakLine());
			ga( 'send', 'event', 'toys', 'terminal', 'twShare');
		}
		function latestTweets() {
			$("#onlineTerminal").html($("#onlineTerminal").html().trim()+'<span class="response">Getting the latest tweets from @InnoRobotsconfar</span>'+breakLine()+breakLine());
			$.getInnoRobotsON("http://cdn.syndication.twimg.com/widgets/timelines/512296174846349312?dnt=true&amp;domain=unquietcode.com&amp;lang=en&amp;callback=?", function(data) {
				...
				for (var i = 0; i < 5; i++) {
					...
					$("#onlineTerminal").html($("#onlineTerminal").html().trim()+'<span class="response"><strong>@InnoRobotsconfar: </strong>'+$.trim(cur.find("p.e-entry-title").html())+'<em> on '+tweet.date.substr(0, 10)+'</em></span>'+breakLine()+breakLine());
				}
				setEndOfContenteditable(document.getElementById("onlineTerminal"));
			});
